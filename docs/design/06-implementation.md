...
> **注意**: このドキュメントは設計書です。一部のライブラリは現在のバージョン (v0.1.2) では未追加です。実際の依存関係については各パッケージの `package.json` を参照してください。
...
#### コアパッケージ (@mosmos_21/vault-key-core)

- **zod**: バリデーション (TypeScript ファーストのスキーマ検証) ✅ 実装済み
- **node:sqlite**: Node.js 組み込み SQLite データベースモジュール (同期 API、軽量、追加パッケージ不要) ✅ 実装済み
- **@simplewebauthn/server**: WebAuthn (Passkey) 認証の実装 (登録・認証フローの生成と検証) ⏳ v0.2.0 で実装予定
- **pg**: PostgreSQL データベースクライアント (非同期 API、トランザクション、コネクションプール) ⏳ v0.4.0 で実装予定

#### CLI パッケージ (@mosmos_21/vault-key-cli)

- **@mosmos_21/vault-key-core**: コアライブラリ ✅ 実装済み
- **commander**: CLI フレームワーク (コマンド定義、オプション解析、ヘルプ生成) ✅ 実装済み
- **chalk**: CLI の色付け出力 (テキストの装飾、エラーメッセージの強調) ✅ 実装済み
- **dotenv**: 環境変数の読み込み (`.env` ファイルから環境変数を読み込み) ✅ 実装済み
- **prompts**: 対話的入力 (パスワード入力、確認ダイアログ) ✅ 実装済み
...
> **実装状況 (v0.1.2)**: PostgreSQL サポートは未実装です。v0.4.0 で実装予定です。
