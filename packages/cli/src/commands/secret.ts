...
export const createSecretCommand = (): Command => {
  ...
  command
    ...
    .action(
      async (key: string, options: { dbPath?: string; token?: string }) => {
        try {
          ...
          console.error(
            formatError(`機密情報の取得に失敗しました: ${message}`),
          );
          process.exit(1);
        }
      },
      ...
      async (key: string, options: { dbPath?: string; token?: string }) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Unknown error';
          console.error(
            formatError(`機密情報の保存に失敗しました: ${message}`),
          );
          process.exit(1);
        }
      },
      ...
      async (key: string, options: { dbPath?: string; token?: string }) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Unknown error';
          console.error(
            formatError(`機密情報の更新に失敗しました: ${message}`),
          );
          process.exit(1);
        }
      },
      ...
      async (key: string, options: { dbPath?: string; token?: string }) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Unknown error';
          console.error(
            formatError(`機密情報の削除に失敗しました: ${message}`),
          );
          process.exit(1);
        }
      },
